...
const Icon = {
  User: () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 4-7 8-7s8 3 8 7"/></svg>,
  ...
  ArrowRight: () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>,
  ...
  DollarSign: () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"></line><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path></svg>,
};

/* ─── Reusable panel wrapper ─────────────────── */
const Panel = ({ eyebrow, title, aside, icon, className = '', children }: any) => (
...
export const AdminPage = ({ session, snapshot, participants, connected, onLogout, onRefreshParticipants }: AdminPageProps) => {
  ...
  // FIX 1: Find the top mover separately without messing up the Quote Deck order
  const topMover = [...snapshot.stocks].sort((a, b) => 
    Math.abs((b.currentPrice - b.basePrice) / (b.basePrice || 1)) - 
    Math.abs((a.currentPrice - a.basePrice) / (a.basePrice || 1))
  )[0];

  // FIX 2: Lock the Quote Deck alphabetically so it NEVER jitters
  const quoteDeck = [...snapshot.stocks].sort((a, b) => a.ticker.localeCompare(b.ticker));
  ...
  return (
    <main className="ap-shell">
      <div className="ap-ambient" aria-hidden="true"><div className="ap-ambient__glow ap-ambient__glow--warm" /><div className="ap-ambient__glow ap-ambient__glow--cool" /></div>
      
      {/* ── Header ── */}
      <header className="ap-header">
        <div className="ap-header__brand">
          <div className="ap-header__mark"><svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M2 20L11 2L20 20" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" /><path d="M5.5 14H16.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" /></svg></div>
          <div><span className="ap-header__name">Venturers Exchange</span><h1 className="ap-header__title">Operations Desk</h1></div>
        </div>
        <div className="ap-header__stats">
          <div className="ap-stat"><span>Operator</span><strong>{session.user?.displayName}</strong></div>
          <div className="ap-stat"><span>Round</span><strong>{snapshot.marketState.currentRoundName ?? 'Not started'}</strong></div>
          <div className={`ap-stat ap-stat--status ap-stat--${tradingStatus}`}><span>Status</span><strong>{tradingStatus === 'open' ? 'Open' : tradingStatus === 'halted' ? 'Halted' : 'Closed'}</strong></div>
          <div className="ap-stat"><span>Board</span><strong>{snapshot.marketState.leaderboardVisible ? 'Visible' : 'Hidden'}</strong></div>
          <div className="ap-stat"><span>Desks</span><strong>{participants.length}</strong></div>
          <div className="ap-stat"><span>Version</span><strong>{snapshot.marketState.eventVersion}</strong></div>
        </div>
        <div className="ap-header__right">
          <div className={`ap-conn ${connected ? '' : 'ap-conn--warn'}`}><span className="ap-conn__dot" /><span>{connected ? 'Control Live' : 'Reconnecting'}</span></div>
          <button className="ap-btn ap-btn--ghost ap-btn--sm" onClick={() => void onLogout()}><Icon.Logout /><span>Logout</span></button>
        </div>
      </header>

      {/* ── Signal strip ── */}
      <section className="ap-signals">
        <article className="ap-signal"><span className="ap-signal__label">Market Breadth</span><strong className="ap-signal__value">{marketBreadth}/{snapshot.stocks.length}</strong><p>Names above base price</p></article>
        <article className="ap-signal"><span className="ap-signal__label">Aggregate Value</span><strong className="ap-signal__value">{formatCurrencyShort(totalPortfolioValue)}</strong><p>Total book value across all desks</p></article>
        <article className="ap-signal"><span className="ap-signal__label">Executed Trades</span><strong className="ap-signal__value">{totalTrades}</strong><p>Running count from leaderboard</p></article>
        <article className="ap-signal"><span className="ap-signal__label">Lead Mover</span><strong className="ap-signal__value">{topMover?.ticker ?? '—'}</strong><p>{topMover ? formatSignedPercent(((topMover.currentPrice - topMover.basePrice) / (topMover.basePrice || 1)) * 100) : 'No data'}</p></article>
      </section>

      {/* ── Toast messages ── */}
      {statusMessage && <div className="ap-toast ap-toast--ok"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg><span>{statusMessage}</span><button onClick={() => setStatusMessage(null)}><Icon.X /></button></div>}
      {errorMessage && <div className="ap-toast ap-toast--err"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><circle cx="12" cy="16" r="0.5" fill="currentColor"/></svg><span>{errorMessage}</span><button onClick={() => setErrorMessage(null)}><Icon.X /></button></div>}
      ...
      <div className="ap-workspace">
        ...
        <div className="ap-col ap-col--left">
          ...
          <Panel eyebrow="Round Control" title="Session Clock" icon={<Icon.Clock />}>
            <div className="ap-field">
              ...
              <select className="ap-select" value={roundId} onChange={(e) => setRoundId(Number(e.target.value))}>{rounds.map((r) => <option key={r.id} value={r.id}>{r.number}. {r.name}</option>)}</select>
            </div>
            <div className="ap-btn-row">
              <button className="ap-btn ap-btn--primary" onClick={() => void runAction(() => request('/api/admin/round/start', { method: 'POST', body: { roundId } }), 'Round started.')}><Icon.Zap /><span>Start Round</span></button>
              <button className="ap-btn ap-btn--ghost" onClick={() => void runAction(() => request('/api/admin/round/end', { method: 'POST' }), 'Round ended.')}>End Round</button>
            </div>
          </Panel>
          ...
          <Panel eyebrow="Safety Layer" title="Emergency Controls" icon={<Icon.Shield />} className="ap-panel--danger-zone">
            ...
            <div className="ap-btn-row">
              <button className="ap-btn ap-btn--danger" onClick={() => void runAction(() => request('/api/admin/halt', { method: 'POST', body: { halted: true, confirmation: haltConfirmation } }), 'Trading halted.')}>Halt Trading</button>
              <button className="ap-btn ap-btn--ghost" onClick={() => void runAction(() => request('/api/admin/halt', { method: 'POST', body: { halted: false } }), 'Trading resumed.')}>Resume</button>
            </div>
            <button className={`ap-btn ap-btn--outline ap-btn--full ${snapshot.marketState.leaderboardVisible ? 'ap-btn--active' : ''}`}
              onClick={() => void runAction(() => request('/api/admin/leaderboard/reveal', { method: 'POST', body: { visible: !snapshot.marketState.leaderboardVisible } }), snapshot.marketState.leaderboardVisible ? 'Leaderboard hidden.' : 'Leaderboard revealed.')}>
              {snapshot.marketState.leaderboardVisible ? <><Icon.EyeOff /><span>Hide Leaderboard</span></> : <><Icon.Eye /><span>Reveal Leaderboard</span></>}
            </button>
          </Panel>

          {/* Manual Cash Adjust Panel */}
          <Panel eyebrow="Desk Funding" title="Manual Cash Adjust" icon={<Icon.DollarSign />} className="ap-panel--narrative">
            <div className="ap-field">
              <label className="ap-field__label">Participant Desk</label>
              {/* USE ALPHABETICAL DESKS HERE SO IT DOESN'T FLICKER */}
              <select className="ap-select" value={adjustUserId} onChange={(e) => setAdjustUserId(Number(e.target.value))}>
                <option value="">-- Select Desk --</option>
                {alphabeticalDesks.map((p) => (
        ...
        <div className="ap-col ap-col--center">
          <Panel eyebrow="Narrative Engine" title="News Trigger" icon={<Icon.Zap />} className="ap-panel--narrative">
            <div className="ap-field"><label className="ap-field__label">Headline</label><input className="ap-input" value={newsHeadline} onChange={(e) => setNewsHeadline(e.target.value)} placeholder="Breaking: Market-moving event..." /></div>
            <div className="ap-field"><label className="ap-field__label">Detail</label><textarea className="ap-textarea" value={newsDetail} onChange={(e) => setNewsDetail(e.target.value)} rows={2} placeholder="Optional context for the event..." /></div>
            ...
            {impacts.length > 0 && (
              <div className="ap-tags">{impacts.map((imp, i) => (<button key={`${imp.ticker}-${i}`} className={`ap-tag ${imp.magnitudePct >= 0 ? 'ap-tag--up' : 'ap-tag--down'}`} onClick={() => setImpacts((c) => c.filter((_, idx) => idx !== i))}>{imp.ticker} {imp.magnitudePct > 0 ? '+' : ''}{imp.magnitudePct}%<Icon.X /></button>))}</div>
            )}
            {pricePreview.length > 0 && (
              <div className="ap-preview">
                <span className="ap-field__label">Price Preview</span>
                {pricePreview.map((p) => (
                  <div key={p.ticker} className="ap-preview__row"><span className="ap-preview__ticker">{p.ticker}</span><span>{formatCurrency(p.from)}</span><Icon.ArrowRight /><strong className={p.pct >= 0 ? 'ap-pct--up' : 'ap-pct--down'}>{formatCurrency(p.to)}</strong><span className={`ap-pct ${p.pct >= 0 ? 'ap-pct--up' : 'ap-pct--down'}`}>{p.pct > 0 ? '+' : ''}{p.pct}%</span></div>
                ))}
              </div>
            )}
            <button className="ap-btn ap-btn--primary ap-btn--full" onClick={() => void runAction(() => request('/api/admin/news', { method: 'POST', body: { headline: newsHeadline, detail: newsDetail, impacts } }), 'News event broadcast.')}><Icon.Send /><span>Broadcast News</span></button>
          </Panel>
          ...
          <Panel eyebrow="Broadcast" title="Floor Message" icon={<Icon.Send />}>
            <textarea className="ap-textarea" rows={3} value={broadcastMessage} onChange={(e) => setBroadcastMessage(e.target.value)} placeholder="Message to all participants..." />
            <button className="ap-btn ap-btn--primary ap-btn--full" onClick={() => void runAction(() => request('/api/admin/broadcast', { method: 'POST', body: { message: broadcastMessage } }), 'Broadcast sent.')}><Icon.Send /><span>Send Broadcast</span></button>
          </Panel>

          <Panel eyebrow="Import" title="Participants from CSV" icon={<Icon.Upload />}>
            <textarea className="ap-textarea ap-textarea--mono" rows={4} value={csv} onChange={(e) => setCsv(e.target.value)} />
            <button className="ap-btn ap-btn--ghost ap-btn--full" onClick={() => void runAction(async () => { const result = await request<{ importedCount: number; usernames: string[] }>('/api/admin/users/import', { method: 'POST', body: { csv } }); setStatusMessage(`Imported ${result.importedCount} users.`); }, 'Users imported.')}><Icon.Upload /><span>Import CSV</span></button>
          </Panel>
        </div>

        {/* ── RIGHT: Leaderboard ── */}
        <div className="ap-col ap-col--right">
          <Panel eyebrow="Room Monitor" title="Leaderboard" icon={<Icon.User />} aside={<span className="ap-badge">{participants.length} desks</span>} className="ap-panel--monitor">
            {participants.length > 0 ? (
              <><div className="ap-lb-top5">{participants.slice(0, 5).map((p) => (<div key={p.userId} className={`ap-lb-podium ap-lb-podium--r${Math.min(p.rank, 4)}`}><span className="ap-lb-podium__rank">#{p.rank}</span><strong className="ap-lb-podium__name">{p.displayName}</strong><span className="ap-lb-podium__val">{formatCurrencyShort(p.portfolioValue)}</span></div>))}</div><div className="ap-lb-list">{participants.slice(0, 15).map((p) => (<article key={p.userId} className="ap-lb-row"><span className="ap-lb-row__rank">#{p.rank}</span><div className="ap-lb-row__info"><strong>{p.displayName}</strong><span>{p.tradeCount} trades</span></div><div className="ap-lb-row__vals"><strong>{formatCurrency(p.portfolioValue)}</strong><span>{formatCurrencyShort(p.cashBalance)} cash</span></div></article>))}</div></>
            ) : (<div className="ap-lb-empty"><p>No participants yet. Import via CSV or wait for sign-ups.</p></div>)}
          </Panel>
        </div>
      </div>
    </main>
